...
type BufferflowSmoothie struct {
	Name      string
	Port      string
	Paused    bool
	BufferMax int
...
func (b *BufferflowSmoothie) Init() {
	b.lock = &sync.Mutex{}
	b.SetPaused(false, 1)

	log.Println("Initting Smoothie buffer flow")
	...
	b.initline, _ = regexp.Compile("^Smoothie")
	b.qry, _ = regexp.Compile("\\M114")
	b.rpt, _ = regexp.Compile("^ok C:")

	//initialize query loop
	b.rptQueryLoop(b.parent_serport) // Disable the query loop
}

func (b *BufferflowSmoothie) RewriteSerialData(cmd string, id string) string {
	return ""
}

func (b *BufferflowSmoothie) BlockUntilReady(cmd string, id string) (bool, bool, string) {
...
func (b *BufferflowSmoothie) OnIncomingData(data string) {
	log.Printf("OnIncomingData() start. data:%q\n", data)

	b.LatestData += data

	//it was found ok was only received with status responses until the Smoothie buffer is full.
	...
	for index, element := range arrLines[:len(arrLines)-1] {
		...
		if b.ok.MatchString(element) || b.err.MatchString(element) {
			...
			if b.q.LenOfCmds() < b.BufferMax {
				...
				log.Printf("Smoothie just completed a line of gcode\n")
			...
			//check for the Smoothie init line indicating the arduino is ready to accept commands
			//could also pull version from this string, if we find a need for that later
		} else if b.initline.MatchString(element) {
			//Smoothie init line received, clear anything from current buffer and unpause
			b.LocalBufferWipe(b.parent_serport)

			//unpause buffer but wipe the command in the queue as Smoothie has restarted.
			if b.GetPaused() {
				b.SetPaused(false, 2)
			}
			...
			//	log.Println("Smoothie status has not changed, not reporting to client")
...
func (b *BufferflowSmoothie) ClearOutSemaphore() {
	ctr := 0
...
func (b *BufferflowSmoothie) BreakApartCommands(cmd string) []string {
	...
	for _, item := range cmds {
		...
		if item == "*init*" { //return init string to update Smoothie widget when already connected to Smoothie
			...
			log.Printf("Wiping Smoothie BufferFlow")
			b.LocalBufferWipe(b.parent_serport)
...
func (b *BufferflowSmoothie) SeeIfSpecificCommandsShouldSkipBuffer(cmd string) bool {
	// remove comments
...
func (b *BufferflowSmoothie) SeeIfSpecificCommandsShouldPauseBuffer(cmd string) bool {
	// remove comments
	//cmd = regexp.MustCompile("\\(.*?\\)").ReplaceAllString(cmd, "")
	//cmd = regexp.MustCompile(";.*").ReplaceAllString(cmd, "")
...
func (b *BufferflowSmoothie) SeeIfSpecificCommandsShouldUnpauseBuffer(cmd string) bool {
...
func (b *BufferflowSmoothie) SeeIfSpecificCommandsShouldWipeBuffer(cmd string) bool {
...
func (b *BufferflowSmoothie) SeeIfSpecificCommandsReturnNoResponse(cmd string) bool {
	/*
		// remove comments
...
func (b *BufferflowSmoothie) ReleaseLock() {
	log.Println("Lock being released in Smoothie buffer")
...
func (b *BufferflowSmoothie) IsBufferGloballySendingBackIncomingData() bool {
...
//This is used for sending query requests outside of the normal buffered operations that will pause to wait for room in the Smoothie buffer
//'?' is asynchronous to the normal buffer load and does not need to be paused when buffer full
func (b *BufferflowSmoothie) rptQueryLoop(p *serport) {
...
func (b *BufferflowSmoothie) Close() {
...
//	go-routine safe.
func (b *BufferflowSmoothie) GetPaused() bool {
	b.lock.Lock()
	defer b.lock.Unlock()
	return b.Paused
}
...
//	go-routine safe.
func (b *BufferflowSmoothie) SetPaused(isPaused bool, semRelease int) {
	b.lock.Lock()
	defer b.lock.Unlock()
	b.Paused = isPaused
...
func (b *BufferflowSmoothie) LocalBufferWipe(p *serport) {
	log.Printf("Pseudo command received to wipe Smoothie buffer but *not* send on to Smoothie controller.")
...
func (b *BufferflowSmoothie) GetManualPaused() bool {
	return false
}

func (b *BufferflowSmoothie) SetManualPaused(isPaused bool) {
}
